...
selectedNames.each { name ->
    ...
    for (i in 1..3) {
        ...
        def outputPath = " -path " + outputDir + "/" + subfolderName
        def outputFile = "channel" + i + "deconv"
        def outputFileString = " -out stack " + outputFile
        def monitor = " -monitor no"
        IJ.run("DeconvolutionLab2 Run", image + psf + algorithm + outputFileString + outputPath + monitor);

        println("Decon params" + image + psf + algorithm + outputFileString + outputPath)
