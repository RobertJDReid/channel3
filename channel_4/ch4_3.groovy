...
//seriesMap = seriesMap.keySet().sort();
//
def seriesNumberMax = seriesMap.keySet().max()
def seriesNumberMin = seriesMap.keySet().min()
println("Series max = " + seriesNumberMax)
println("Series min = " + seriesNumberMin)

def imgRange = seriesNumberMin..seriesNumberMax
def seriesList = imgRange.toList()

// Need to make a list from 'Series 0' .. 'Series Max' to get values for dialog

GenericDialog gd = new GenericDialog("Select Series to Process")
//def seriesNames = seriesList
seriesList.each { n ->
